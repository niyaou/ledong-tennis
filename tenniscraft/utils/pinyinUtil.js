...
export var dict = {} // 存储所有字典数据

export var pinyinUtil =
{
  /**
...
export function pySort (arr) {
  ...
  for (var i = 0; i < letters.length; i++) {
    ...
    for (var j = 0; j < arr.length; j++) {
      ...
      var firstLetter = pinyinUtil.getFirstLetter(name)
      var initial = firstLetter.charAt(0)// 截取第一个字符
      if (initial === letters[i] || initial === letters[i].toLowerCase()) {
        if (!nameLetter.includes(initial.toUpperCase())) {
          nameLetter.push(initial.toUpperCase())
        }
        curr.data.push({
          // Name: arr[j].nickName
          Name: name
...
function parsePolyphone (chinese, result, splitter, withtone) {
  ...
  if (temp) // 如果直接找到了结果
  {
    ...
    for (var i1 = 0; i1 < temp.length; i1++) {
      result[i1] = temp[i1] || result[i1]
      if (!withtone) result[i1] = pinyinUtil.removeTone(result[i1])
    }
