...
public class DoublePickRandomMatch extends DoubleMatchStrategy {
    ...
    public DoublePickRandomMatch(RedisUtil redis, IRankService rankService) {
        this.redis = redis;
    ...
    public String matchingGame(DoubleMatchRequestVo vo) {
        ...
        redis.hdel(DoubleMatchRequestVo.DOUBLEMATCHREQUESTFROM, StringUtil.combiningSpecifiedUserKey(user, null));
        // child key
        // expired
        // time specification
        ...
        Map<Object, Object> map = redis.hmget(DoubleMatchRequestVo.DOUBLEMATCHREQUESTFROM);
        if (map.size() != 0) {
            for (Map.Entry<Object, Object> entry : map.entrySet()) {
                if (redis.lockKey((String) entry.getKey(), 30)) {
                    ...
                    String matchId = postDoubleMatches(null, otherRequest.getUserName(),null, user,null,
