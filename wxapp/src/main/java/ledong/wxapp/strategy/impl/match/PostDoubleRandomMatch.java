...
public class PostDoubleRandomMatch extends DoubleMatchStrategy {
    ...
    public PostDoubleRandomMatch(RedisUtil redis) {
        this.redis = redis;
    }

    @Override
    public String matchingGame(DoubleMatchRequestVo vo) {
        redis.hset(DoubleMatchRequestVo.DOUBLEMATCHREQUESTFROM, StringUtil.combiningSpecifiedUserKey(vo.getUserName(), null), vo,
                10);
        long interval = 6000;
        int count = 0;
