...
void closeStream(srs_rtmp_t rtmp)
{
	srs_rtmp_destroy(rtmp);

	cout << "srs_rtmp_destroy";
...
int RealDataCallBack_V2(long lRealHandle, const PACKET_INFO_EX *pFrame, long dwUser)
{
	...
	try
	{
        // int *pts = srs_utils_time_ms()*(1000/20);
		 inc = inc+50;
		...
		// }
		int ret = srs_h264_write_raw_frames(rtmp, pFrame->pPacketBuffer, pFrame->dwPacketSize, inc, inc);
        if (ret != 0) {
		...
		// int ret = -1;
    // printf(" push stream result: %d, iframe-type:%d ,iframe rate: %d ,buffer size : %d, buffer seq: %d, timestamp:%d,pts timestamp : %d\n",
	// ret,pFrame->nPacketType,pFrame->dwFrameRate,pFrame->dwPacketSize,
	// pFrame->dwFrameNum,pFrame->dwTimeStamp,inc);
...
int main(void)
{
	...
	int nError = 0;
#ifdef INCLUDE_NAT
	g_LoginID = H264_DVR_Login_Cloud((char *)"2bfefe4c6bcf4889", 34567, (char *)"admin", (char *)"12", (LPH264_DVR_DEVICEINFO)(&OutDev), &nError, NULL);
#else
	g_LoginID = H264_DVR_Login((char *)"10.217.6.33", 34567, (char *)"admin", (char *)"", (LPH264_DVR_DEVICEINFO)(&OutDev), &nError);
#endif
	printf("g_LoginID=%d,nError:%d\n", g_LoginID, nError);
	printf("sEncryptVersion=%s ,sSoftWareVersion =%s, time restrict=%d,\n", OutDev.sEncryptVersion, OutDev.sSoftWareVersion,OutDev.status[2]);
	...
	if (g_LoginID > 0)
	{
		...
		const char *rtmp_url = "rtmp://10.219.14.54:1935/live/test";
		const char *raw_file = "test h264";
		double fps = 25.0;
...
#define RealPlay
		...
		else
		{
			...
			printf("start RealPlay ok! sleep 1000");
			sleep(1000);
			if (H264_DVR_StopRealPlay(m_iPlayhandle))
			{
				printf("force  stop realPlay ok\n");
			}
			else
			{
